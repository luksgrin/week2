...
describe('Custom Tests', function () {
  ...
  it('[assignment] ii. deposit 0.1 ETH in L1 -> withdraw 0.08 ETH in L2 -> assert balances', async () => {
    ...
    await transaction({
      ...
      recipient: recipient,
      isL1Withdrawal: false, // False, so withdrawal is not in L1
    })
  ...
  it('[assignment] iii. see assignment doc for details', async () => {
    ...
    await transaction({
      ...
      isL1Withdrawal: false // because it's L2
    })

    // Up to this point, everything is similar to the transact test in `full.test.js`

    // Make Alice withdraw to L1. Similar to the `deposit to L1 and withdraw to L1` test in `full.test.js`
    const aliceWallet = new ethers.Wallet(aliceDepositUtxo.keypair.privkey)
    const aliceEthAddress = aliceWallet.address;
    const TPBalanceAfterBob = await token.balanceOf(tornadoPool.address)
    const aliceWithdrawAmount = aliceDepositAmount.sub(bobWithdrawAmount)

    const aliceWithdrawUtxo = new Utxo({
      amount: TPBalanceAfterBob.sub(aliceWithdrawAmount),
      keypair: aliceDepositUtxo.keypair,
    })
